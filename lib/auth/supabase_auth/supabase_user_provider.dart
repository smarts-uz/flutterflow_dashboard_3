...
class Dashboard03TestAppSupabaseUser extends BaseAuthUser {
  Dashboard03TestAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dashboard03TestAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = Dashboard03TestAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
